...
from OGGYMUSIC import YouTube, app
from OGGYMUSIC.core.call import Oggy
from OGGYMUSIC.misc import SUDOERS, db
from OGGYMUSIC.utils.database import (is_active_chat,
                                       is_music_playing, is_muted,
                                       is_nonadmin_chat, music_off,
                                       music_on, mute_off, mute_on,
                                       set_loop)
from OGGYMUSIC.utils.decorators.language import languageCB
from OGGYMUSIC.utils.formatters import seconds_to_min
from OGGYMUSIC.utils.inline.play import (panel_markup_1,
                                          panel_markup_2,
                                          panel_markup_3,
                                          stream_markup,
                                          telegram_markup)
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Oggy.pause_stream(chat_id)
        ...
        await Oggy.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await oggy.mute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await oggy.unmute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        if "live_" in queued:
            ...
            try:
                await Oggy.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            try:
                await Oggy.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
